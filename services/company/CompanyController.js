...
router.patch('/:companyId/logo',upload.single('logo'), async (req, res) => {
    try {
        const {companyId} = req.params;
        const {file} = req;
        const s3Response = await uploadFile(file, `company/${companyId}/logo`).then((data) => {
            unlink(file.path);
        })
 ...
 * update backdrop
 */
router.patch('/:companyId/backDrop',upload.single('backdrop'), async (req, res) => {
    try {
        const {companyId} = req.params;
        const {file} = req;
        const s3Response = await uploadFile(file, `company/${companyId}/backdrop`).then((data) => {
            unlink(file.path);
        })
        const company = await Company.findById(companyId);
...
router.patch('/:companyId/photo',upload.single('photo'), async (req, res) => {
    try {
        ...
        await company.save();
        const s3Response = await uploadFile(file, `company/${companyId}/photo/${photoId}`).then((data) => {
            unlink(file.path);
        })
        res.status(200).send(`${process.env.SERVER_DOMAIN}/api/v1/company/${companyId}/photo/${photoId}`)
    } catch (e) {
        console.log(e);
